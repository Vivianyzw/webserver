...
class FaceDetector(object):
    def __init__(self, model_path=r'./model/FaceDetector.onnx'):
    ...
    def draw_result(self, image, detections):
        thickness = image.shape[0] // 1000 + 1
        for d in detections:
            if self.vis_threshold > d[4]:
                continue
            cv2.rectangle(image, (int(d[0]), int(d[1])), (int(d[2]), int(d[3])), (255, 255, 255), thickness=thickness)
        return image
...
if __name__ == '__main__':
    ...
    handle = FaceDetector()
    detections, landmarks = handle.run(image)
    print(detections)
    image = handle.draw_result(image, detections)
    cv2.imshow('', image)
    cv2.waitKey()
