...
class FaceLandmark(object):
    def __init__(self, model_path=r'./model/FaceLandmark.onnx'):
    ...
    def run(self, image, bboxes):
        ...
        return np.array(all_landmarks), np.array(all_states)

    def show_result(self, image, landmarks):
        thickness = image.shape[0] // 1000 + 1
        for landmark in landmarks:
            for point in landmark:
                cv2.circle(image, center=(int(point[0]), int(point[1])),
                           color=(255, 255, 255), radius=1, thickness=thickness)
        cv2.imshow('', image)
        cv2.waitKey()

    def draw_result(self, image, landmarks):
        thickness = image.shape[0] // 1200 + 1
        for landmark in landmarks:
            for point in landmark:
                cv2.circle(image, center=(int(point[0]), int(point[1])),
                           color=(255, 255, 255), radius=2, thickness=thickness)
        return image
...
if __name__ == '__main__':
    ...
    bbox = np.array([[495, 194, 1063,  1000]])
    handle = FaceLandmark()
    landmarks, _ = handle.run(image, bbox)
    image = handle.draw_result(image, landmarks)
    cv2.imshow('', image)
    cv2.waitKey()
    cv2.imwrite('landmark.jpg', image)
